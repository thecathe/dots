...
local menubar = require("menubar") -- local menubar = require("menubar") -- <-- default
...
local batteryarc_widget = require('awesome-wm-widgets.batteryarc-widget.batteryarc')    -- https://github.com/streetturtle/awesome-wm-widgets/tree/master/battery-widget
local logout_menu_widget = require('awesome-wm-widgets.logout-menu-widget.logout-menu') -- https://github.com/streetturtle/awesome-wm-widgets/tree/master/logout-menu-widget

-- load things for device specific setups
local device_spec = require("device_spec")
...
terminal = "alacritty"                    -- xterm <-- default
editor = os.getenv("EDITOR") or "emacs"
editor_cmd = terminal .. " -e " .. editor -- nano <-- default
...
mytextclock = wibox.widget.textclock("%Y %m %d | %H:%M", 10)
...
-- local notification_status = awful.widget.button()
-- local notification_status = awful.widget.button()
-- notification_status:buttons(gears.table.join(
...
notification_status:connect_signal("button::press", function(c)
  ...
  if naughty.is_suspended() then
    ...
    c.checked = false
    naughty.notify {
      text = "turning notifications off",
      timeout = 1,
      urgency = "critical",
      ontop = true,
      destroy = function() naughty.suspend() end
    }
  end
  -- c.checked = naughty.is_suspended()
end)
-- naughty.


local wibox_sep = wibox.widget.textbox(" ")
...
awful.screen.connect_for_each_screen(function(s)
  ...
  s.mywibox:setup {
    ...
    {             -- Right widgets
      ...
      -- ! volume_widget(),
      volume_widget {
        widget_type = 'vertical_bar' -- <-- arc, vertical/horizontal_bar
      },
      wibox_sep,
      -- only show batery widget if device has battery (i.e., is laptop)
      (device_spec.has_battery and (batteryarc_widget {
...
globalkeys = gears.table.join(
  ...
  awful.key({ modkey, "Shift" }, "s", function() awful.spawn("flameshot gui") end,
    { description = "flamshot screenshot", group = "applications" }),
  ...
  awful.key({}, "XF86MonBrightnessUp", function()
      local inc = 'xbacklight -inc 3 ; sleep 0.1 ; xbacklight -get'
    ...
    -- only show shortcut if device has brightness (i.e., is laptop)
    (device_spec.has_brightness and { description = "brighter", group = "screen brightness" } or {})
  ),
  awful.key({}, "XF86MonBrightnessDown", function()
    ...
    -- only show shortcut if device has brightness (i.e., is laptop)
    (device_spec.has_brightness and { description = "dimmer", group = "screen brightness" } or {})
  ),
  ...
  awful.key({ modkey, "Shift" }, "XF86MonBrightnessUp", function()
    ...
    -- only show shortcut if device has brightness (i.e., is laptop)
    (device_spec.has_brightness and { description = "bright", group = "screen brightness" } or {})
  ),
  awful.key({ modkey, "Shift" }, "XF86MonBrightnessDown", function()
    ...
    -- only show shortcut if device has brightness (i.e., is laptop)
    (device_spec.has_brightness and { description = "dim", group = "screen brightness" } or {})
  ),
  ...
  awful.key({ modkey, "Control" }, "XF86MonBrightnessUp", function()
    ...
    -- only show shortcut if device has brightness (i.e., is laptop)
    (device_spec.has_brightness and { description = "brightest", group = "screen brightness" } or {})
  ),
  awful.key({ modkey, "Control" }, "XF86MonBrightnessDown", function()
    ...
    -- only show shortcut if device has brightness (i.e., is laptop)
    (device_spec.has_brightness and { description = "dimmest", group = "screen brightness" } or {})
  ),

  -- temperature hotkeys (toggle color warmth)
  awful.key({ modkey, "Control" }, "Right", function()
      local redshift_val = 2700
  ...
  awful.key({ modkey }, "q", function()
      -- naughty.toggle()
      -- notification_status.checked = naughty.is_suspended()
      if naughty.is_suspended() then
        notification_status.checked = true
        naughty.resume()
        naughty.notify {
          text = "notifications on",
          timeout = 2,
        ...
        notification_status.checked = false
        naughty.notify {
          text = "turning notifications off",
          timeout = 1,
          urgency = "critical",
  ...
  awful.key({ modkey, "Control" }, "s", function() xrandr.xrandr() end,
    { description = "cycle through xrandr configs", group = "screen" }),
  awful.key({ modkey, "Shift", "Control" }, "s", function() awful.spawn("arandr") end,
    { description = "open arandr", group = "screen" })
)
...
awful.rules.rules = {
  ...
  -- Firefox Picture-in-Picture
  {
    ...
    properties = { skip_taskbar = true, titlebars_enabled = true, urgent = true, role = "Picture-in-Picture", maximized = false, above = true, floating = true, sticky = false }
  },
}
-- }}}
...
-- Autostart: https://wiki.archlinux.org/title/Awesome#Autostart
awful.spawn.with_shell(
  'if (xrdb -query | grep -q "^awesome\\.started:\\s*true$"); then exit; fi;' ..
  'xrdb -merge <<< "awesome.started:true";' ..
  -- insert autostart commands here (separated by ; single quotes,)
...
-- calculator
calc = function(...)
  local restore_suspension = false
  if naughty.is_suspended() then
    restore_suspension = true
    naughty.resume()
  end
  naughty.notify {
    title = table.concat({ ... }, '\t'),
    text = "= " .. ...,
    timeout = 2,
    destroy = function() if restore_suspension then naughty.suspend() end end
  }
end
-- show color
col = function(col)
  ...
  local str_col
  if string.sub(tostring(col), 1, 1) == "#" then
    str_col = tostring(col)
  else
    str_col = string.format("#%s", tostring(col))
  end
  naughty.notify {
    text = tostring(str_col),
    -- text = "    ",
    bg = str_col,
    timeout = 0,
    urgency = "low",
...
-- {{{ init?
-- following: https://www.reddit.com/r/awesomewm/comments/syjolb/comment/hy0xy35/?utm_source=share&utm_medium=web3x&utm_name=web3xcss&utm_term=1&utm_content=share_button
